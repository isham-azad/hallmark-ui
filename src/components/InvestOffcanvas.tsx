...
export default function InvestOffcanvas() {
    const { isInvestOpen, setIsInvestOpen } = useInvest();
            ...
            if (element && bootstrap) {
                ...
                if (isInvestOpen) {
                    ...
                    setTimeout(() => {
                        ...
                        if (backdrops.length > 0 && !isInvestOpen) {
                ...
                const handleHidden = () => setIsInvestOpen(false);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch("/api/site/enquiry", {
                ...
                body: JSON.stringify({
                    ...formData,
                    message: `Investor Interest Query\nState: ${formData.state}\nCity: ${formData.city}\nPIN: ${formData.pin}`,
    ...
    return (
        <div
            ...
            id="investOffcanvas"
            ref={offcanvasRef}
            aria-labelledby="investOffcanvasLabel"
            style={{ width: "450px" }}
        >
            <div className="offcanvas-header" style={{ backgroundColor: '#111', borderBottom: '1px solid #333' }}>
                <h5 className="offcanvas-title fw-bold text-white d-flex align-items-center" id="investOffcanvasLabel">
                    <span className="d-flex align-items-center justify-content-center rounded-circle me-3" style={{ width: '40px', height: '40px', backgroundColor: 'rgba(255, 196, 81, 0.1)' }}>
                        <i className="bi bi-graph-up-arrow" style={{ color: '#ffc451' }}></i>
                    </span>
                    Investor Relations
                </h5>
                <button
            ...
            <div className="offcanvas-body p-4" style={{ backgroundColor: '#f8f9fa' }}>
                {submitted ? (
                    <div className="text-center py-5" data-aos="fade-in">
                        ...
                        <p className="text-muted">Thank you for your interest in Hallmark's growth journey. Our team will get in touch with you shortly.</p>
                        <button
                    ...
                    <form onSubmit={handleSubmit} className="invest-form bg-white p-4 rounded-4 shadow-sm border border-light">
                        <div className="mb-4 text-center p-3 rounded-3" style={{ backgroundColor: '#111', color: '#fff' }}>
                            <h6 className="fw-bold mb-1 text-uppercase" style={{ color: '#ffc451', letterSpacing: '1px', fontSize: '0.85rem' }}>Partner with Hallmark</h6>
                            <p className="small text-white-50 mb-0">Join our journey of expansion and excellence. Leave your details below.</p>
                        </div>
                        ...
                        <p className="text-center mt-4 x-small text-muted mb-0">
                            ...
                            Your investment interest is confidential and secure.
                        </p>
                    </form>
                )}
            </div>
