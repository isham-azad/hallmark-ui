    ...
    return (
        <div className="product-card service-item position-relative d-flex flex-column h-100" style={{ padding: '0', overflow: 'hidden', border: '1px solid color-mix(in srgb, var(--default-color), transparent 90%)', borderRadius: '15px', transition: 'all 0.4s ease', backgroundColor: 'var(--surface-color)', boxShadow: '0 5px 15px rgba(0,0,0,0.05)' }}>
            <Link href={`/product/${product.id}`} className="product-card-img" style={{ overflow: 'hidden', position: 'relative', display: 'block' }}>
                <img
                    src={(product.image?.split(',')[0]) || `/assets/img/masonry-portfolio/masonry-portfolio-${(index % 9) + 1}.jpg`}
                    alt={product.title}
                    className="img-fluid"
                    style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.6s ease' }}
                />
                <div className="category-badge" style={{ position: 'absolute', top: '15px', left: '15px', backgroundColor: 'rgba(255, 255, 255, 0.95)', padding: '5px 12px', borderRadius: '30px', fontSize: '0.65rem', fontWeight: '700', color: 'var(--accent-color)', textTransform: 'uppercase', letterSpacing: '1px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', zIndex: '2' }}>
                    {product.category.replace("-", " ")}
                </div>
            </Link>
            <div className="product-card-content p-3 p-md-4 d-flex flex-column flex-grow-1">
                <h3 className="product-card-title" style={{ fontWeight: "700", marginBottom: "12px", textAlign: "left", color: 'var(--heading-color)', transition: '0.3s' }}>{product.title}</h3>
                ...
                <Link href={`/product/${product.id}`} className="product-card-link mt-3 pt-3 border-top d-flex align-items-center" style={{ color: 'var(--accent-color)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.5px', textDecoration: 'none' }}>
