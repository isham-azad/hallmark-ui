...
async function handler(request: Request, { logAction }: { logAction: any }) {
    ...
    const validation = CategoryUpdateSchema.safeParse({ id, name, summary });
    ...
    await logAction("UPDATE_CATEGORY", { categoryId: id, name });

    return NextResponse.json({ success: true });
}

export const POST = withAdminAuth(handler, PERMISSIONS.MANAGE_CATEGORIES);
